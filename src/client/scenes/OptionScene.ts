...
export class OptionScene extends BaseScene {
  ...
  public override create() {
    ...
    // BERSIHKAN UI LAMA JIKA SCENE DIPAKAI LAGI
    try {
      this.rows.forEach(r => r.destroy());
    } catch {}
    this.rows = [];
    this.closeModal(); // pastikan tidak ada modal sisa dari sesi sebelumnya
  ...
  private createModalItem(
    ...
    const txt = this.add
      .text(0, 0, label, {
        ...
        color: '#000',
        align: 'center',
        wordWrap: { width: Math.floor(widthPx * 0.92) },
      })
      .setOrigin(0.5)
      .setDepth(depth + 1);
  ...
  private buildModalBox(
    titleText: string,
    headerLines: string[] = [],
    innerButtons: { label: string; onTap: () => void }[] = [],
  ) {
    this.closeModal(); // pastikan tidak ada modal lama
    ...
    const buttonH = Math.max(
      40,
      Math.round(Math.min(this.scale.width, this.scale.height) * 0.05),
    );
    const gap = Math.round(buttonH * 0.18);
    const padding = Math.max(
      18,
      Math.round(Math.min(this.scale.width, this.scale.height) * 0.028),
    );
    const titleSize = Math.max(
      20,
      Math.round(Math.min(this.scale.width, this.scale.height) * 0.038),
    );

    const totalButtonsHeight =
      innerButtons.length * buttonH +
      (innerButtons.length > 0 ? (innerButtons.length - 1) * gap : 0);
    const headerBlockHeight = headerLines.length
      ? headerLines.length * Math.round(titleSize * 0.9) +
        Math.round(titleSize * 0.5)
      : 0;
    ...
    const boxHeight = Math.min(
      boxContentHeight,
      Math.round(this.scale.height * 0.75),
    );
    ...
    const title = this.add
      .text(
        this.centerX,
        this.centerY - boxHeight / 2 + padding,
        titleText,
        {
          fontFamily: 'Nunito',
    ...
    headerLines.forEach(line => {
      const hdr = this.add
        .text(this.centerX, cursorY, line, {
          fontFamily: 'Nunito',
          fontSize: `${Math.max(
            14,
            Math.round(titleSize * 0.88),
          )}px`,
          color: '#000',
        })
        .setOrigin(0.5)
    ...
    innerButtons.forEach(btn => {
      const pill = this.createModalItem(
        btn.label,
        btn.onTap,
        innerWidth,
        buttonH,
  ...
  private openVersionModal() {
    ...
    this.buildModalBox(
      ...
      versions.map(v => ({
        label: v,
        onTap: () => {
          SettingsManager.save({ version: normalizeVersion(v) });
          this.refreshLabels();
          this.closeModal();
        },
      })),
