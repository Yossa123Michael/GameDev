...
export class Game extends BaseScene {
    constructor() {
        ...
        this.lives = 0;
        // timer
        this.sessionTimeRemaining = 0; // belajar
        this.perQuestionRemaining = 0; // survive
    ...
    getAnswerIndex(q) {
        return q.answerIndex ?? q.correctAnswerIndex ?? -1;
    }
    createOptionButton(centerY, label, onClick) {
    ...
    highlightCorrect(correctIdx) {
        ...
        if (!btn)
            return;
        const width = btn.width ?? 200;
        const height = btn.height ?? 48;
        const radius = Math.min(24, Math.floor(height * 0.35));
        const g = btn.getAt(0);
    ...
    draw() {
        ...
        if (this.activeOptionButtons.length > 0) {
            ...
            this.activeOptionButtons.forEach((btn, i) => {
                ...
                btn.height = height;
                const g = btn.getAt(0);
                const txt = btn.getAt(1);
                const zone = btn.getAt(2);
                this.updateButtonGraphics(g, width, height, 0xffffff, 0x000000, 3, radius);
                txt.setStyle({
                    fontSize: `${Math.max(14, Math.floor(height * 0.38))}px`,
                    wordWrap: { width: Math.floor(width * 0.9) },
