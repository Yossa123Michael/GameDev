...
export type SubmitResult = {
  ...
  reason?: 'not_higher' | 'no_user' | 'error';
  data?: any;
  error?: string;
};
...
function setLastSubmission(user_id: string | null, name: string, score: number, created_at: string) {
  try { localStorage.setItem(LS_KEY_LAST, JSON.stringify({ user_id, name, score, created_at })); } catch {}
}

export function getLastSubmission() {
  try { const raw = localStorage.getItem(LS_KEY_LAST); return raw ? JSON.parse(raw) : null; } catch { return null; }
}

function getDeviceBest(): number | null {
  try { const raw = localStorage.getItem(LS_KEY_BEST); return raw ? Number(raw) : null; } catch { return null; }
}

function setDeviceBestIfHigher(score: number) {
  try {
    const cur = getDeviceBest();
    if (cur == null || score > cur) localStorage.setItem(LS_KEY_BEST, String(score));
  } catch {}
}
...
async function waitForUserId(maxMs = 5000, stepMs = 150): Promise<string | null> {
  ...
  try { const { data } = await supabase.auth.getUser(); return (data as any)?.user?.id ?? null; } catch { return null; }
}
...
export async function submitScoreViaFunction(
  ...
  try {
    ...
    if (bestRow) currentBest = { score: (bestRow as any).score, created_at: (bestRow as any).created_at };
  } catch (e) {
